...
public class EmployeeServiceImpl implements EmployeeService {
     ...
     *   {
     *      原理分析步骤：
     *          1.找到XxxAutoConfiguration并进入
     *          2.关注@Import注解
     *          3.打印容器组件注册信息（debug: true）
     *          4.关注默认注册的组件
     *   }
     ...
     *
     */
    @Cacheable(value = "emp",key="#id",cacheManager = "")
//    @Cacheable(value = "emp",keyGenerator = "myKeyGenerator")
     ...
     * 注意：
     *  1.更新（缓存）之前和更新之后的数据类型要一致
     *      getEmpById-->Employee 必须updateEmp-->Employee
     *
     */
    ...
    @Caching(
            ...
            put={
                @CachePut(value = "emp",key="#result.get(0).id"),
                @CachePut(value = "emp",key="#result.get(0).email")
            }
    )
    @Override
